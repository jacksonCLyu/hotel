...
class SqlDEVPExamine extends React.Component {
    ...
    render() {
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (text, record) => (
                    <div>
                        <Link to={`/SqlDEVP/getDEVPExamine/${record.id}`}>查看</Link>
                    </div>
                )
            }
        ];
...
const mapStateToProps = (state) => {
    const { list, total, page } = state.sqlDEVPExamine;
    return {
        loading: state.loading.models.sqlDEVPExamine,
        list,
        total,
        page
    };
}
SqlDEVPExamine = Form.create()(SqlDEVPExamine);
export default connect(mapStateToProps)(SqlDEVPExamine);
